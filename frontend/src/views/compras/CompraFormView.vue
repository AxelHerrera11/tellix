<template>
    <div class="page">
      ...
      <div class="pos-grid">
        <!-- Panel izquierdo: búsquedas y carrito -->
        <div class="pos-left">
          ...
          <div class="card">
            <div class="form-row">
              <div class="form-group form-group--half">
                <label class="label">No. documento (factura proveedor)</label>
                <input v-model="form.noDocumento" type="text" placeholder="Ej: FAC-001" class="field-input" />
              </div>
              <div class="form-group form-group--half">
            ...
            <div v-if="form.fkMetodoPago === 2" class="plazo-row">
              <label class="label">Plazo de crédito (días)</label>
              <input v-model.number="form.plazoCredito" type="number" min="1" class="field-input" style="max-width:120px" />
            </div>
          </div>
          ...
          <div class="card">
            ...
            <div class="search-row">
              ...
              />
              <button class="btn btn-ghost btn-sm" @click="proveedorSeleccionado = null; form.fkRepresentante = null">
                Limpiar
              </button>
            </div>
            ...
            <!-- Representante -->
            <div v-if="proveedorSeleccionado" class="rep-row">
              <label class="label">Representante (opcional)</label>
              <select v-model="form.fkRepresentante" class="field-input">
                <option :value="null">Sin representante</option>
                <option v-for="r in representantes" :key="r.nit" :value="r.nit">
                  {{ r.nombreCompleto }}
                </option>
              </select>
            </div>
          ...
          <div class="card">
            ...
            <div v-if="productosResultados.length" class="dropdown">
              ...
              >
                <span class="d-main">{{ p.nombre }}</span>
                <span class="d-sub">Stock actual: {{ p.stockActual }} {{ p.medida }}</span>
              </div>
            </div>
          </div>
        ...
        <!-- Panel derecho: totales y registro -->
        <div class="pos-right">
          <!-- Totales -->
...
import { compraService, type ProductoCompra, type ProveedorCompra, type RepresentanteCompra } from '@/services/compra.service'
...
const representantes = ref<RepresentanteCompra[]>([])
...
const form = ref({
  ...
  fkMetodoPago:   1,
  plazoCredito:   0,
  fkRepresentante: null as string | null
})
...
const puedeRegistrar = computed(() =>
  !!proveedorSeleccionado.value && form.value.noDocumento.trim().length > 0 && carrito.value.length > 0
)

watch(() => form.value.fkMetodoPago, v => {
  if (v !== 2) form.value.plazoCredito = 0
})

watch(proveedorSeleccionado, async (prov) => {
  if (prov) {
    form.value.fkRepresentante = null
    try {
      representantes.value = await compraService.listarRepresentantes(prov.nit)
    } catch {
...
async function registrar() {
  ...
  const precioInvalido = carrito.value.find(i => i.precioUnitario <= 0)
  if (precioInvalido) {
    errorMsg.value = `El precio unitario de "${precioInvalido.nombre}" debe ser mayor a cero.`
    return
  }
  ...
  try {
    ...
    const id = await compraService.registrar({
      ...
      fkRepresentante: form.value.fkRepresentante,
...
.rep-row { margin-top:10px }

.plazo-row { margin-top:8px }
...
.td-sub { font-variant-numeric:tabular-nums }
.qty-input { width:80px; padding:.3rem .5rem; border:1px solid var(--color-border); border-radius:6px; font-size:.85rem; background:var(--color-bg-input); color:var(--color-text) }
