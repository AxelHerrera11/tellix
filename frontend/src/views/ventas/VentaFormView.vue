        ...
        <div class="pos-left">
          ...
          <div class="card">
            ...
            <div class="search-row">
              ...
              />
              <button class="btn btn-ghost btn-sm" @click="clienteSeleccionado = null">
                Limpiar
              </button>
            </div>
            <div v-if="clientesResultados.length && !clienteSeleccionado" class="dropdown">
              <div
                v-for="c in clientesResultados"
                :key="c.nit"
                class="dropdown-item"
                @click="seleccionarCliente(c)"
              >
                <span class="d-main">{{ c.nombre }}</span>
                <span class="d-sub">{{ c.nit }}</span>
              </div>
            </div>
            <div v-if="clienteSeleccionado" class="cliente-chip">
              <span class="chip-nombre">{{ clienteSeleccionado.nombre }}</span>
              <span class="chip-nit">{{ clienteSeleccionado.nit }}</span>
            </div>
          </div>
          ...
          <div class="card">
            ...
            <div v-if="productosResultados.length" class="dropdown">
              ...
              >
                ...
                <span class="d-sub">
                  Stock: {{ p.stockActual }} {{ p.medida }} —
                  Q {{ formatNum(p.precioVenta) }}
                </span>
              </div>
            </div>
          </div>
          ...
          <div class="card card--grow">
            <label class="label">Detalle de la venta</label>
            ...
            <table v-else class="cart-table">
              ...
              <tbody>
                <tr v-for="(item, i) in carrito" :key="i">
                  ...
                  <td class="td-precio">Q {{ formatNum(item.precioUnitario) }}</td>
          ...
          <div class="card">
            ...
            <div v-if="form.fkMetodoPago === 2" class="plazo-row">
              <div class="plazo-field">
                <label class="label-sm">Plazo</label>
                <input v-model.number="form.plazoCredito" type="number" min="1" class="field-input" />
              </div>
              <div class="plazo-field">
                <label class="label-sm">Unidad</label>
                <select v-model="form.tipoPlazo" class="field-input">
...
import { ventaService, type ProductoVenta, type ClienteVenta } from '@/services/venta.service'
...
const clientesResultados = ref<ClienteVenta[]>([])
const clienteSeleccionado = ref<ClienteVenta | null>(null)

const productoBusqueda   = ref('')
const productosResultados = ref<ProductoVenta[]>([])
...
const puedeRegistrar = computed(() =>
  !!clienteSeleccionado.value && carrito.value.length > 0
)
...
// ── Carrito ───────────────────────────────────────────────────
function seleccionarCliente(c: ClienteVenta) {
...
async function registrar() {
  errorMsg.value = ''
  if (!clienteSeleccionado.value) { errorMsg.value = 'Seleccione un cliente.'; return }
...
.plazo-row { display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:10px }
