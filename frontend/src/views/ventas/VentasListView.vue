<template>
    <div class="page">
      <div class="page-header">
        ...
        <router-link to="/ventas/nueva" class="btn btn-primary">+ Nueva venta</router-link>
      </div>
      ...
      <div class="table-card">
        ...
        <div v-else-if="!ventas.length" class="table-empty">No se encontraron ventas.</div>
        <table v-else class="data-table">
          <thead>
            <tr>
              <th>#</th><th>Cliente</th><th>Fecha</th><th>Total</th><th>Método</th><th>Estado</th><th></th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="v in ventas" :key="v.id">
              <td class="td-id">{{ v.id }}</td>
              <td><div class="cell-main">{{ v.nombreCliente || v.fkCliente }}</div></td>
              <td>{{ formatFecha(v.fechaOperacion) }}</td>
              <td class="td-monto">Q {{ formatNum(v.total) }}</td>
              <td>{{ v.metodoPago }}</td>
              <td><span class="badge" :class="`badge--${v.estado.toLowerCase()}`">{{ v.estadoDescripcion }}</span></td>
              <td class="td-actions"><router-link :to="`/ventas/${v.id}`" class="action-btn">Ver</router-link></td>
            </tr>
          </tbody>
        </table>
      </div>
...
import { ventaService, type VentaResumen, type FiltrosVenta } from '@/services/venta.service'
import type { PagedResponse } from '@/services/api'
import { useToast } from '@/composables/useToast'

const toast    = useToast()
const ventas   = ref<VentaResumen[]>([])
const paginado = ref<PagedResponse<VentaResumen> | null>(null)
const cargando = ref(false)
const pagina   = ref(1)
const filtros  = ref<FiltrosVenta>({ cliente: '', estado: '', desde: '', hasta: '' })
