...
export const cxcService = {
  ...
  async vencidas(fecha?: string): Promise<CxcVencida[]> {
    const params = new URLSearchParams()
    if (fecha) params.append('fecha', fecha)
    const { data } = await api.get<ApiResponse<CxcVencida[]>>(`/cxc/vencidas?${params.toString()}`)
    return data.data
  },
  async obtener(id: number): Promise<CxcDetalle> { const { data } = await api.get<ApiResponse<CxcDetalle>>(`/cxc/${id}`); return data.data },
  async registrarCobro(id: number, request: RegistrarCobroRequest): Promise<void> { await api.post(`/cxc/${id}/cobros`, request) },
  async anular(id: number, request?: AnularCxcRequest): Promise<void> { await api.patch(`/cxc/${id}/anular`, request ?? {}) },
