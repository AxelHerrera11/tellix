...
public class CompraRepository {
        ...
        return jdbc.execute((Connection con) -> {
            try (CallableStatement cs = con.prepareCall(
                ...
                CompraDto.CompraDetalle completa = new CompraDto.CompraDetalle(
                    ...
                    cabecera.fkProveedor(), cabecera.proveedor(),
                    cabecera.direccionFiscal(),
                    cabecera.fkRepresentante(), cabecera.representante(),
                    ...
                    cabecera.nombreEmpleado(), cabecera.creadoEn(), items
                );
    ...
    public int registrar(CompraDto.CrearCompraRequest req, int fkUsuario) {
        ...
        return jdbc.execute((Connection con) -> {
            try (CallableStatement cs = con.prepareCall(
                "EXEC sp_registrar_compra @p_no_documento=?, @p_proveedor=?, @p_representante=?, @p_usuario=?, @p_metodo_pago=?, @p_plazo=?, @p_items=?"
            )) {
                cs.setString(1, req.noDocumento());
                cs.setString(2, req.fkProveedor());
                cs.setString(3, req.fkRepresentante());
                cs.setInt(4,    fkUsuario);
                cs.setInt(5,    req.fkMetodoPago());
                cs.setInt(6,    req.plazoCredito());
                ...
                throw new TellixException("No se pudo registrar la compra.", HttpStatus.INTERNAL_SERVER_ERROR);
            }
        });
    }
    ...
    public List<CompraDto.RepresentanteCompra> listarRepresentantes(String proveedor) {
        return jdbc.execute((Connection con) -> {
            try (CallableStatement cs = con.prepareCall(
                "EXEC sp_listar_representantes @p_proveedor=?"
            )) {
                cs.setString(1, proveedor);

                List<CompraDto.RepresentanteCompra> lista = new ArrayList<>();
                boolean hasResult = cs.execute();
                while (true) {
                    if (hasResult) {
                        try (ResultSet rs = cs.getResultSet()) {
                            while (rs.next()) {
                                lista.add(new CompraDto.RepresentanteCompra(
                                    rs.getString("nit"),
                                    rs.getInt("codigo"),
